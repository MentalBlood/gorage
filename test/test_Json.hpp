...
TEST_CASE("encoding to JSON") {
	...
	SUBCASE("dictionaries") {
		...
		SUBCASE("hardcoded full comparison") { CHECK_EQ(encoded, "{\"double\":" "1234.123400,\"int\":" "1234,\"string\":" "\"lalala\"}"); }
	}
}
TEST_CASE("JSON stability") {
	...
	SUBCASE("lists") {
		SUBCASE("->decoding->encoding->") {
			...
			for (size_t i = 0; i < l.size(); i++) CHECK_EQ(std::any_cast<int>(l[i]), std::any_cast<int>(result[i]));
		}
		SUBCASE("->encoding->decoding->") {
	...
	SUBCASE("dictionaries") {
		SUBCASE("->decoding->encoding->") {
			...
			for (const auto& k_v : d) CHECK_EQ(std::any_cast<int>(d.at(k_v.first)), std::any_cast<int>(result.at(k_v.first)));
			for (const auto& k_v : result) CHECK_EQ(std::any_cast<int>(d.at(k_v.first)), std::any_cast<int>(result.at(k_v.first)));
		}
		SUBCASE("->encoding->decoding->") {
...
void testJsonSearchingSimpleDict(const std::any& value) {
	CHECK_EQ(gorage::Json::contains(gorage::Json::Dict({{"key", value}}), "key", value), true);
}
TEST_CASE("JSON searching") {
	...
	SUBCASE("dictionaries") {
		...
		testJsonSearchingSimpleDict(gorage::Json::Dict({{"", ""}, {"1", "1"}, {"2", 2}, {"3.4", 3.4}}));
	}
}