...
namespace gorage {
	...
	public:
		...
		void write(const T& object) const {
			...
			if (path.parent_path().string().length() && !std::filesystem::exists(path.parent_path())) std::filesystem::create_directories(path.parent_path());

			if constexpr (std::is_same_v<T, Bytes>) {
				std::ofstream file(path, std::ios::trunc | std::ios::binary);
				if (!file.is_open()) throw exceptions::CanNotWriteFile(path);
				...
				if constexpr (std::is_same_v<T, std::string>) file << object; else file << object.encoded();
				file.close();
			}
		}
		T read() const {
			if constexpr (std::is_same_v<T, Bytes>) {
				...
				if (!file.is_open()) throw exceptions::CanNotReadFile(path);
