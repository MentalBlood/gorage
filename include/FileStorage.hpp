...
namespace gorage {
	...
	public:
		...
		explicit Storage(const std::filesystem::path& folder, const std::string& extension): folder(folder), extension(extension) {}

		std::filesystem::path path(const Usi& usi) const { return (folder / usi.value()).replace_extension(extension); }
		...
		virtual std::vector<Usi> usis() const {
			...
			for (const auto& p : std::filesystem::directory_iterator(folder))
				if (p.path().extension() == extension) result.push_back(Usi(p.path().stem().string()));
