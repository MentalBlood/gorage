...
namespace gorage {
	namespace exceptions {
		...
		public:
			KeyError(const gorage::Usi& usi): Base("No object with usi `" + usi.value() + "`") {}
		};
	}
	template<class T>
	...
	public:
		...
		virtual T load(const Usi& usi) const {
			if (!exists(usi)) throw exceptions::KeyError(usi);
		...
		virtual bool exists(const Usi& usi) const { return _storage.count(usi.value()); }

		virtual Bytes raw(const Usi& usi) const { throw exceptions::NotImplemented("MemoryStorage", "raw"); }

		virtual std::vector<Usi> usis() const {
			std::vector<Usi> result;
			for (const auto& pair : _storage) result.push_back(Usi(pair.first));
			return result;
		}
	private:
