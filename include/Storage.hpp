...
namespace gorage {
	namespace exceptions {
		...
		public:
			explicit NoSuchPart(const Usi& usi, const size_t number): Base("Object with USI " + usi.value() + "have no part of number " + std::to_string(number)) {}
		};
		class CanNotBuild : public Base {
		public:
			explicit CanNotBuild(const size_t number): Base("Can not build object from given " + std::to_string(number) + " parts") {};
		};
	}
	...
	public:
		...
		virtual P load(const Usi& usi, size_t part_number) const {
			if (part_number == 0) return load(usi);
			else throw exceptions::NoSuchPart(usi, part_number);
		}
		virtual T build(const std::vector<P> parts) {
			if (parts.size() == 1) return parts[0];
			else throw exceptions::CanNotBuild(parts.size());
		}
		...
		T load(const Usi& usi, T default_) {
			try { return load(usi); }
			catch(const exceptions::Base& e) { throw; }
			catch(const std::exception& e) { return default_; }
		}
		virtual void remove(const Usi& usi) = 0;

		void clear() { for (const Usi& usi : usis()) remove(usi); }
